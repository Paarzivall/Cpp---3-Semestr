...
int main(){
	...
	klasa_tab *w = new klasa_tab(tablica);// alokacja pamięci dla obiektu
	assert(w);
	
	w->drukuj();//tutaj wywołujemy metodę drukuj
	
	int x = 0;
	std::cout << "\nPodaj liczbe ktora chcesz znalezc:\t";
	std::cin >> x;
	bool a = w->szukaj(x);// wywołujemy metodę szukaj :)
	...
	delete w;//zwolnienie zaalokowanej pamięci
	return 0;
}
